            ...
            <div className="max-w-3xl mx-auto space-y-8 p-4 md:p-8 w-full">
                ...
                <div className="bg-[#1A1A2E] border rounded-xl overflow-hidden shadow-sm">
                    <div className="p-6 border-b">
                        <h2 className="text-xl font-semibold mb-1 text-[#E5E7EB]">General Information</h2>
                        <p className="text-lg text-[#9CA3AF] mb-6">Update your workspace name and URL slug.</p>

                        <form action={boundUpdateAction} className="space-y-4">
                            <div>
                                <label htmlFor="name" className="block text-lg font-medium text-[#E5E7EB] mb-1">Workspace Name</label>
                                <input
                                    type="text"
                                    id="name"
                                    name="name"
                                    ...
                                    disabled={!isAdmin}
                                    className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none ${!isAdmin && 'bg-[#0F0F1A] text-[#9CA3AF] cursor-not-allowed'}`}
                                />
                            </div>

                            <div>
                                <label htmlFor="slug" className="block text-lg font-medium text-[#E5E7EB] mb-1">URL Slug</label>
                                <div className="flex">
                                    <span className="inline-flex items-center px-4 rounded-l-lg border border-r-0 bg-[#0F0F1A] text-[#9CA3AF] sm:text-lg">
                                        /dashboard/
                                    </span>
                                    <input
                                        type="text"
                                        ...
                                        disabled={!isAdmin}
                                        className={`flex-1 w-full px-4 py-2 border rounded-none rounded-r-lg focus:ring-2 focus:ring-blue-500 outline-none ${!isAdmin && 'bg-[#0F0F1A] text-[#9CA3AF] cursor-not-allowed'}`}
                                    />
                                </div>
                            </div>
                            ...
                            {!isAdmin && (
                                <p className="text-lg text-[#9CA3AF] italic mt-2">Only administrators can modify these settings.</p>
                            )}
                        </form>
                    </div>
                </div>
                    ...
                    <div className="border border-red-200 rounded-xl overflow-hidden shadow-sm bg-red-50/30">
                        <div className="p-6">
                            ...
                            <form action={boundDeleteAction} className="bg-[#1A1A2E] border border-red-100 p-5 rounded-lg space-y-4">
                                <div>
                                    <label htmlFor="confirmText" className="block text-lg font-medium text-[#E5E7EB] mb-1">
