...
export function Sidebar({ workspaceName, workspaceSlug, role }: { workspaceName: string, workspaceSlug: string, role: string }) {
  ...
  return (
    <aside className="w-64 bg-slate-900 text-slate-300 flex flex-col hidden md:flex">
      {/* Workspace Branding */}
      <div className="p-4 h-16 flex items-center border-b border-slate-800 bg-slate-950">
      ...
      <div className="p-4 border-t border-slate-800 bg-slate-950/50 flex items-center justify-between">
        ...
        <span className={`text-xs px-2 py-1 rounded-full font-bold ${role === 'ADMIN' ? 'bg-purple-500/20 text-purple-400' : 'bg-slate-700 text-slate-300'}`}>
          {role}
        </span>
      </div>
    </aside>
